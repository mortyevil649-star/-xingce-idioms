...
export function IdiomCard({ idiom }: { idiom: Idiom }) {
  ...
  return <article className="paper paper-hover relative min-w-0 rounded-2xl p-4 sm:p-5">
    {user ? <button
      ...
      className={`focus-ring absolute right-3 top-3 flex size-11 items-center justify-center rounded-full border sm:right-4 sm:top-4 ${record?.is_favorite ? 'status-favorite' : 'status-idle'}`}
    >
      ...
      aria-label="登录后收藏"
      className="focus-ring status-idle absolute right-3 top-3 flex size-11 items-center justify-center rounded-full sm:right-4 sm:top-4"
    >
      <Bookmark size={18} />
    </Link>}
    <Link to={`/idioms/${idiom.id}`} className="block min-w-0 pr-12">
      <div className="flex min-w-0 flex-wrap items-center gap-2">
        ...
        {hasNote && <span className="chip status-review">有笔记</span>}
      </div>
      ...
      <div className="mt-4 flex min-w-0 flex-wrap gap-2">
        ...
        {frequencyLabel && <span className="chip status-review">{frequencyLabel}</span>}
    ...
    {user ? <button
      ...
      className={`focus-ring mt-5 flex min-h-11 w-full items-center justify-center gap-2 rounded-xl border px-3 py-2.5 text-sm font-bold ${mastered ? 'status-mastered' : 'status-idle hover:bg-emerald-50'}`}
    >
