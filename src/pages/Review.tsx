...
export function Review() {
  ...
  return <div className="mx-auto w-full max-w-4xl">
    <div className="paper rounded-2xl p-5 sm:rounded-3xl sm:p-8">
      ...
      <p className="mt-3 text-[15px] leading-7 text-slate-500">复习范围只包含已掌握成语。本组 {count} 个，预计约 {estimatedMinutes} 分钟。</p>
      {!user && <div className="mt-5 rounded-2xl bg-indigo-50 p-4"><p className="mb-3 text-sm leading-6 text-slate-600">登录后才能读取你的已掌握成语并同步复习结果。</p><LoginRequiredLink className="w-full sm:w-auto" /></div>}
    </div>
    ...
    {user && !pool.length && <div className="paper mt-5 rounded-2xl p-8 text-center text-slate-500">
      ...
      <p className="mt-4">暂时没有已掌握成语可复习。可以先去成语库学习并标记“已掌握”。</p>
    ...
    {user && Boolean(pool.length) && <div className="mt-5 grid gap-4">
      {pool.map((idiom, index) => <article key={idiom.id} className="paper min-w-0 rounded-2xl p-5 sm:p-6">
